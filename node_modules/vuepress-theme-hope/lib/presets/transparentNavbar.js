...
const encodeDataURI = (content) => content
    .replace(/"/g, "'")
    .replace(/%/g, "%25")
    .replace(/#/g, "%23")
    .replace(/{/g, "%7B")
 ...
 * 将导航栏设置为透明
 */
export const setupTransparentNavbar = ({ type, threshold = 50, light = "#fff", dark = "#bbb", } = {}) => {
    const route = useRoute();
    const routeLocale = useRouteLocale();
    const frontmatter = usePageFrontmatter();
    const shouldTransparent = computed(type === "all"
        ? () => true
        : type === "homepage"
    ...
    useStyleTag((type === "homepage" ? HOMEPAGE_STYLE : BLOG_HOMEPAGE_STYLE) +
        (light === dark
            ? getStyle(light, "")
